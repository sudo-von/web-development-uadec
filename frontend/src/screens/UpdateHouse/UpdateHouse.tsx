...
import handleStyles from './UpdateHouse.styles';
...
const UpdateHouse = (): JSX.Element => {
  ...
  const statesOptions: Option[] = states.map(
    ({ id: i, Description: stateDescription }) => ({
      key: stateDescription,
      value: i,
    }),
  );
  const citiesOptions: Option[] = cities.map(
    ({ id: i, Description: cityDescription }) => ({
      key: cityDescription,
      value: i,
    }),
  );
  ...
  const handleSubmit = async (): Promise<void> => {
    if (
      ...
      try {
        if (id) {
          const housePayload: UpdateHousePayload = {
            id: id,
          ...
          await updateHouse(housePayload);
          swal('Has actualizado la casa con éxito');
        }
      } catch (error) {
        swal(
  ...
  return (
    <Skeleton>
      ...
      >
        <h1 style={styles.title}>Actualiza una casa</h1>
        <form action="POST">
          ...
          <Container justifyContent="center" style={styles.buttonContainer}>
            <Button onClick={handleSubmit}>Actualizar casa</Button>
          </Container>
        </form>
      </Container>
    </Skeleton>
